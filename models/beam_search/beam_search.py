...
class BeamSearch(object):
    ...
    def _expand_state(self, selected_beam, cur_beam_size):
        def fn(s):
            ...
            beam = torch.tensor(beam, dtype=torch.int64, device=beam.device)
    ...
    def _expand_visual(self, visual: utils.TensorOrSequence, cur_beam_size: int, selected_beam: torch.Tensor):
        if isinstance(visual, torch.Tensor):
            ...
            selected_beam_exp = torch.tensor(selected_beam_exp, dtype=torch.int64, device=selected_beam_exp.device)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        ...
        selected_beam = torch.tensor(selected_beam, dtype=torch.int64, device=selected_beam.device)
        ...
        selected_words = torch.tensor(selected_words, dtype=torch.int64, device=selected_words.device)
